...
def initialize():
    ...
    while True:
        ...
        while True:
            # def yesno is on line 94
            multi = yesno(inp=input('Multiplayer? [Y/N/RESET/QUIT]:  '))
            ...
            if any(multi):
                ...
                # pass on to P1First call loop. multi input should be correct.
                break
            else:
                continue
        ...
        while True:
            P1First = yesno(
                inp=input('P1, wanna go first? [Y/N/RESET/QUIT]:  '))
...
def playermoved(Board, currentPMark, otherPMark):
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                moveOrigin = input(
                    'Type in a slot number where you have one of your pieces [or Q=QUIT]:  ')
        ...
        while True:
            ...
            try:
                ...
                moveDest = input(
                    'Choose a slot number where you want it to go [or R=RETURN / Q=QUIT]:  ')
                ...
                # If there is a jump, and the slot in between has a player's piece on it; a forbidden move in Siga.
    ...
    # I use the move origin to make sure later that the player moved all the three pieces before winning, another rule of Sega.
    return moveOrigin
...
def compumoved(Board, currentPMark, otherPMark, allmoved):
    ...
    for square in Board:
        ...
        # else if the square has a piece that belongs to computer
    ...
    for winset in win:
        ...
        if len(set(moveOrigins).intersection(winset)) == 2:  # This means that if TWO of my pieces are in a win set go on
            ...
            winsquare = winset.difference(set(moveOrigins)).pop()
            # The final piece's ideal position to get a win
    ...
    if winmoving:  # if there is a chance to win
        ...
        else:  # There's only one case I'm aware of that this code will execute: 
			# That is if there was an unmoved piece that was also trapped from all directions
            # So, using AIlist here instead of options because options in this case would be an empty list []
...
def wincheck(Board, player, allmoved=False):
    ...
    # If all the player-in-question's pieces moved, check if he won. Because you have to move all your pieces in Siga before a line of pieces is considered a win.
    if allmoved:
...
def runGame():
    ...
    while True:
        ...
        if info['Multiplayer']:  # if multiplayer is true
            ...
            # all pieces didn't move: False False False
            ...
            if info['P1First']:  # if player 1 goes first is True
                ...
                while True:
					...
					# playermoved returns the move origin which is used here to say that the original piece's place has been abandoned at least once, even if it's filled again.
                    ...
                    # all(P1moved.values()) means that all the pieces have at least moved once, if so the value would be True.
                        showBoard(Board)
                        print('P1 Won!')
                        break
